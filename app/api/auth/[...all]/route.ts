...
const botOptions = {
  mode: "LIVE",
  // Block automated bots from submitting the form
  // https://arcjet.com/bot-list
  deny: [], // Empty array means no specific bots are denied, but bot detection is still active
...
async function protect(req: NextRequest): Promise<ArcjetDecision> {
  ...
  // If the user is logged in we'll use their ID as the fingerprint. This
  ...
  if (session?.user.id) {
    ...
    fingerprint = ip(req) || "127.0.0.1"; // Fall back to local IP if none
  }
